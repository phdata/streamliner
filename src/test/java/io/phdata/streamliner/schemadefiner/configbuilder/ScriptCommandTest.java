...
public class ScriptCommandTest {
  ...
  public void testScriptCommandForSchemaEvolution_Alter_Table_Add_column(){
    ...
    Configuration prevConfig =
            StreamlinerUtil.createConfig(previousDirectory, StreamlinerUtil.readYamlFile(config), "--previous-state-directory");
    Configuration currConfig = StreamlinerUtil.createConfig(stateDirectory, StreamlinerUtil.readYamlFile(config), "--state-directory");
    ConfigurationDiff diff = DiffGenerator.createConfigDiff(prevConfig, currConfig);
    StreamlinerUtil.writeConfigToYaml(diff, String.format("%s/%s", stateDirectory, Constants.STREAMLINER_DIFF_FILE.value()));
  ...
  public void testScriptCommandForSchemaEvolution_Alter_Table_Modify_column() {
    ...
    String previousDirectory = "src/test/resources/configDiff/changeColumn/previous-state-directory";
    Configuration prevConfig =
            StreamlinerUtil.createConfig(previousDirectory, StreamlinerUtil.readYamlFile(config), "--previous-state-directory");
    Configuration currConfig = StreamlinerUtil.createConfig(stateDirectory, StreamlinerUtil.readYamlFile(config), "--state-directory");
    ConfigurationDiff diff = DiffGenerator.createConfigDiff(prevConfig, currConfig);
  ...
  public void testScriptCommandToGenerateScripts(){
    ...
    String previousDirectory = "src/test/resources/configDiff/addAllColumn/previous-state-directory";
    outputDir =
  ...
  public void testScriptCommand_mandatory_config() {
    expectedEx.expect(RuntimeException.class);
    expectedEx.expectMessage("--state-directory has no table config and streamliner-diff.yml not found.");
  ...
  public void testScriptCommandForSchemaEvolution_column_size_increased() {
    ...
    String previousDirectory = "src/test/resources/configDiff/columnSizeChanged/previous-state-directory";
    Configuration prevConfig = StreamlinerUtil.createConfig(previousDirectory, StreamlinerUtil.readYamlFile(config), "--previous-state-directory");
    Configuration currConfig = StreamlinerUtil.createConfig(stateDirectory, StreamlinerUtil.readYamlFile(config), "--state-directory");
    ConfigurationDiff diff = DiffGenerator.createConfigDiff(prevConfig, currConfig);
    StreamlinerUtil.writeConfigToYaml(diff, String.format("%s/%s", stateDirectory,Constants.STREAMLINER_DIFF_FILE.value()));
  ...
  public void testScriptCommandForSchemaEvolution_column_comment_nullable_changed() {
    String stateDirectory = "src/test/resources/configDiff/columnCommentAndNullable/state-directory";
    String previousDirectory = "src/test/resources/configDiff/columnCommentAndNullable/previous-state-directory";
    Configuration prevConfig = StreamlinerUtil.createConfig(previousDirectory, StreamlinerUtil.readYamlFile(config), "--previous-state-directory");
    Configuration currConfig = StreamlinerUtil.createConfig(stateDirectory, StreamlinerUtil.readYamlFile(config), "--state-directory");
    ConfigurationDiff diff = DiffGenerator.createConfigDiff(prevConfig, currConfig);
    StreamlinerUtil.writeConfigToYaml(diff, String.format("%s/%s", stateDirectory,Constants.STREAMLINER_DIFF_FILE.value()));
  ...
  public void testScriptCommandForSchemaEvolution_no_schema_changes() {
    ...
    String previousDirectory = "src/test/resources/configDiff/noSchemaChanges/previous-state-directory";
    Configuration prevConfig = StreamlinerUtil.createConfig(previousDirectory, StreamlinerUtil.readYamlFile(config), "--previous-state-directory");
    Configuration currConfig = StreamlinerUtil.createConfig(stateDirectory, StreamlinerUtil.readYamlFile(config), "--state-directory");
    ConfigurationDiff diff = DiffGenerator.createConfigDiff(prevConfig, currConfig);
    StreamlinerUtil.writeConfigToYaml(diff, String.format("%s/%s", stateDirectory,Constants.STREAMLINER_DIFF_FILE.value()));
